...
public class Curator<T> implements Supplier<T> {
	...
	public Curator(String prefix, Conf<T> conf, CuratorFramework curator, Function<byte[], T> decode) {
		this(prefix, conf, curator, decode, v -> {});
	}
	...
	public Curator(String prefix, Conf<T> conf, CuratorFramework curator, Function<byte[], T> decode,
		...
		this.delegate = new LocalSupplier<>(() -> {
			try {
				...
				nodecache.rebuild();
				return Optional.ofNullable(nodecache.getCurrentData()).map(ChildData::getData).map(this.decode)
	...
	public void set(byte[] bytes) {
		try {
			...
			try {
				this.curator.create().creatingParentsIfNeeded().withMode(CreateMode.PERSISTENT).forPath(this.path,
						bytes);
